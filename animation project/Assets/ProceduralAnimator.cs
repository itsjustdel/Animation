...
;
    ...
    void Pelvises()
    {
        ...
        centre -= root.transform.forward * footLength/2;//? keep, variable?
        ...
        if(!leftIdle)
            leftPelvis.transform.position = centre - gyro.transform.right * waistWidth * .5f + lean * root.transform.forward;
        if (!rightIdle)
            rightPelvis.transform.position = centre + gyro.transform.right * waistWidth * .5f + lean * root.transform.forward;

        /*
        //clamp --- was using this for a ling time, remove safe??
        if (!leftIdle)
        ...
        //put gyro at root position, it's the rotation of the gyro we want, not particularly the position
        gyro.transform.position = root.transform.position;
    }  
        ...
        else if (edges.Count > 0)
        {
            ...
            if (i == 0)
            {
                ...
                if (!edgeNearOtherFootFound)
                {
                    ...
                    {
                        //place toe at edge, put foot slightly on step
                        rightFootTarget = edges[edges.Count - 1] + root.transform.forward * footLength / 2;
            ...
            else if (i == 1)
            {
                ...
                if (!edgeNearOtherFootFound)
                {
                    ...
                    {
                        Debug.Log("Stepping on edge");
                        leftFootTarget = edges[edges.Count - 1] + root.transform.forward * footLength / 2;
